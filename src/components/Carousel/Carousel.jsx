...
const Carousel = () => {
    ...
    const imagesPerSlice = 4
    const totalSlices = Math.ceil(data.length / imagesPerSlice)
    const [currentSlice, setCurrentSlice] = useState(0)

    const next = () => {
        setCurrentSlice((currentSlice + 1) % totalSlices)
    }

    const prev = () => {
        setCurrentSlice((currentSlice - 1 + totalSlices) % totalSlices)
    }

    const sliceStartIndex = currentSlice * imagesPerSlice
    const sliceEndIndex = sliceStartIndex + imagesPerSlice
    const slicedData = data.slice(sliceStartIndex, sliceEndIndex)
    ...
    }, [isAutoPlayEnabled, currentSlice])

    return (
        <div className='carousel'>
            <button onClick={prev}>{'<'}</button>
            <div className='carouselImgs' onMouseEnter={() => setAutoPlayEnabled(false)} onMouseLeave={() => setAutoPlayEnabled(true)}>
                {slicedData.map((item) => (
